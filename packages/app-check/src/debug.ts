...
declare global {
  interface Window {
    /**
     * When it is a string, we treat it as the debug token and give it to the consumers as the app check token
     ...
     * When it is `undefined`, `false` or any unsupported value type, the SDK will operate in the production mode
     */
...
export async function getDebugToken(): Promise<string> {
  ...
  if (state.enabled && state.token) {
    ...
    // should not happen!
    throw Error(`
            Can't get debug token in the production mode.
        `);
  }
}

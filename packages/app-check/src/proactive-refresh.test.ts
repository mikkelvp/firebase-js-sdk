...
describe('proactive refresh', () => {
  ...
  it('can be stopped during wait', () => {
    ...
    expect(operation.promise).to.eventually.rejectedWith('not resolved');
    expect(refresher.isRunning()).to.be.false;
    clock.restore();
  });
  ...
  it('can be restarted after being stopped', () => {
    ...
    refresher.start();
    clock.tick(waitTime);
    expect(operationAfterRestart.promise).to.eventually.fulfilled;
